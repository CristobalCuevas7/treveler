...
class LoginUseCase {
  ...
  LoginUseCase(this._authRepository, this._userRepository, this._cleanUserDataUseCase);

  Future<Result<MyUser>> execute(String email, String password) async {
    try {
      Result<MyUser> userResult = await _authRepository.loginUser(email, password);
